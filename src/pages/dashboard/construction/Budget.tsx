  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Monitor project budgets and expenses"
        icon={DollarSign}
        actions={
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: 'Total Budget', value: formatCurrency(stats.totalBudget), icon: DollarSign, color: CONSTRUCTION_COLOR },
          { label: 'Total Spent', value: formatCurrency(stats.totalSpent), icon: TrendingUp, color: '#3b82f6' },
          { label: 'Committed', value: formatCurrency(stats.totalCommitted), icon: PieChart, color: '#8b5cf6' },
          { label: 'Remaining', value: formatCurrency(stats.totalRemaining), icon: DollarSign, color: '#10b981' },
          { label: 'Variance', value: formatCurrency(stats.variance), icon: stats.variance >= 0 ? TrendingUp : TrendingDown, color: stats.variance >= 0 ? '#10b981' : '#ef4444' },
        ].map((stat, index) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="p-4">
            <h3 className="font-semibold text-text-primary mb-4">Budget Breakdown by Category</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-background-tertiary">
                  <tr>
                    <th className="text-left py-3 px-4 text-sm font-medium text-text-muted">Category</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-text-muted">Budgeted</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-text-muted">Spent</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-text-muted">Committed</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-text-muted">Remaining</th>
                    <th className="text-center py-3 px-4 text-sm font-medium text-text-muted">% Spent</th>
                  </tr>
                </thead>
                <tbody>
                ...
                <tfoot className="bg-background-tertiary">
                  <tr>
                    <td className="py-3 px-4 font-semibold text-text-primary">Total</td>
        ...
        <div className="space-y-6">
          <Card className="p-4">
            <h3 className="font-semibold text-text-primary mb-4">Budget Distribution</h3>
          ...
          <Card className="p-4">
            <h3 className="font-semibold text-text-primary mb-4">Project Summary</h3>
            {selectedProjectData && (
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-border-default">
                  {stats.totalSpent / stats.totalBudget > selectedProjectData.completion / 100 ? (
                    <div className="flex items-center gap-2 text-warning">
                      ...
                      <span className="text-sm">Spending ahead of progress</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 text-success">
                      <TrendingUp size={16} />
                      <span className="text-sm">Budget on track</span>
                    </div>
                  )}
                </div>
              </div>
